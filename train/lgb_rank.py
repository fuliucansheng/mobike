...
x.sort_values(["orderid"],inplace=True)
y.sort_values(["orderid"],inplace=True)
xgroup = x.groupby("orderid",as_index=False)["userid"].count()['userid'].values.reshape(-1,)
ygroup = y.groupby("orderid",as_index=False)["userid"].count()['userid'].values.reshape(-1,)

x = lgb.Dataset(x[predictors],label = x['label'],group=xgroup)
y = lgb.Dataset(y[predictors],label = y['label'],group=ygroup)

gbm = lgb.train(params, x, num_round, early_stopping_rounds=20, valid_sets=[x,y])
gc.collect()
train_feat.loc[:,'pred'] = gbm.predict(train_feat[gbm.feature_name()])
gc.collect()
result = reshape(train_feat)
score = map_score(result)
logging.info(score)
gbm.save_model(Config.cache_dir + "/lgb_rank_%s_%s.txt"%(feature_num,score))
